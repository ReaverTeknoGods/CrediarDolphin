...
void MainWindow::StartGame(std::unique_ptr<BootParameters>&& parameters)
{
  ...
  if (parameters && std::holds_alternative<BootParameters::NANDTitle>(parameters->parameters))
  {
    ...
    if (std::get<BootParameters::NANDTitle>(parameters->parameters).id == Titles::SYSTEM_MENU)
    {
      ...
      {
        ModalMessageBox::critical(
            this, tr("Error"), tr("You can't use the SP1 in Wii mode."), QMessageBox::Ok);
        HideRenderWidget();
        return;
      }
      ...
      {
        ModalMessageBox::critical(this, tr("Error"), tr("Please enable Emulated Memory Size Override."),
                                  QMessageBox::Ok);
        HideRenderWidget();
        return;
      }
